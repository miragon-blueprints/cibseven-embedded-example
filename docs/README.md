...
the dev loop and the smoke test in [CONTRIBUTING.md](../CONTRIBUTING.md); the day-to-day agent
workflow in [AGENTS.md](../AGENTS.md). Where a procedure has a decision behind it, that reasoning is
...
| [0002](adr/0002-hexagonal-architecture-for-the-backend.md) | Hexagonal architecture for the backend, machine-enforced by ArchUnit + Checkstyle. |
| [0003](adr/0003-openapi-as-the-checked-in-contract.md) | OpenAPI as the checked-in, drift-gated contract for any consumer. |
...
| [0007](adr/0007-two-architecture-test-tools-archunit-and-konsist.md) | Two architecture-test tools: ArchUnit (bytecode) and Konsist (source). *(Java port: Konsist replaced by Checkstyle — see the ADR's update note.)* |
...
| [0011](adr/0011-build-and-deployment-approach.md) | Build & deployment: `spring-boot:build-image` OCI image for the headless app. |
